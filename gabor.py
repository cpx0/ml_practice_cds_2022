...
def G_env(t, amp, sdev):
    '''
    The impact of the filter is controlled by
    a Gaussian function, the "envelope".
    '''
...
def G_fil_real(t, paras):
    ...
    '''
    carrier = G_car_real(t=t, freq=paras["freq"], phase=paras["phase"])
    envelope = G_env(t=t, amp=paras["amp"], sdev=paras["sdev"])
...
def G_fil_imag(t, paras):
    ...
    Assumes that t is an array of temporal inputs.
    '''
    carrier = G_car_imag(t=t, freq=paras["freq"], phase=paras["phase"])
    envelope = G_env(t=t, amp=paras["amp"], sdev=paras["sdev"])
    out = carrier * envelope
    return out
...
def my_signal(t, hf=5.0, lf=0.25):
    '''
    A custom signal.
    '''
    
    highfreq = 0.25 * np.sin((2*math.pi*hf*t))
    lowfreq = 2 * np.sin((2*math.pi*lf*t))
...
def G2_car_real(x, y, freqx, freqy, phase):
    '''
...
def G2_car_imag(x, y, freqx, freqy, phase):
    '''
...
def G2_fil_real(x, y, paras):
    ...
    # Computations.
    carrier = G2_car_real(x=x, y=y, freqx=u, freqy=v, phase=paras["phase"])
    envelope = G2_env(x=x, y=y, amp=paras["amp"], sdev=paras["sdev"])
    out = carrier * envelope
    return out
...
def G2_fil_imag(x, y, paras):
    ...
    v = paras["freqs"] * math.sin(paras["dir"])
    # Computations.
    carrier = G2_car_imag(x=x, y=y, freqx=u, freqy=v, phase=paras["phase"])
    envelope = G2_env(x=x, y=y, amp=paras["amp"], sdev=paras["sdev"])
    out = carrier * envelope
    return out


def G2_fil_kernel(paras, n_stds=3):
    '''
    ...
    v = paras["freqs"] * math.sin(paras["dir"])
    # Computations.
    envelope = G2_env(x=x, y=y,
                      amp=paras["amp"]/(2*math.pi*paras["sdev"]**2),
                      sdev=paras["sdev"])
    out = {"real": None, "imag": None}
    out["real"] = envelope * G2_car_real(x=x, y=y, freqx=u, freqy=v, phase=paras["phase"])
    out["imag"] = envelope * G2_car_imag(x=x, y=y, freqx=u, freqy=v, phase=paras["phase"])
...
def G2_getfeatures(ims, fil_paras, gridshape, mode="reflect", cval=0):
    ...
    fil_values = G2_fil_kernel(paras=fil_paras, n_stds=2)
    ...
    for i in range(num_ims):
        ...
        # Convolution.
        resp_fil_real = ndi.convolve(input=im,
                                     weights=fil_values["real"],
                                     mode=mode, cval=cval)
        resp_fil_imag = ndi.convolve(input=im,
